...
function getPageContent() {
    ...
    if (webpageContent.length > 1300) {
        webpageContent = webpageContent.slice(1300);
    }
    console.log(webpageContent);
    allWebPageData = `Seperate the part that matches html tags , css code , js and analyse the main content of what the the topic is talking about donot give any code or staff like that and Give Summary of the text in 50 words, the innertext is ${webpageContent}`;
...
async function searchAnythingWithquery(payloadData, tabId) {
    let url = `https://www.google.com/search?q=${payloadData}`;
    try {
        ...
        chrome.tabs.onActivated.addListener(executeContentScript());
    } catch (e) {
        console.log(e);
    }
}
...
function searchFromBookmark(payloadData) {
    ...
    for (let i = 0; i < bookmarksArray.length; i++) {
        ...
        console.log(element);
        if (
            element.title.toLowerCase().includes(payloadData) ||
            element.url.toLowerCase().includes(payloadData)
        ) {
            // console.log(1);
            link = element.url;
            console.log(link);
    ...
    if (link) {
        ...
        chrome.runtime.sendMessage({
            prompt: "Opened the desiered Tab From Bookmark",
        });
    } else {
...
async function closeTabs(tabId) {
    try {
        await chrome.tabs.remove(tabId, function () {
...
async function makeAPIRequestForChat(modeActivation) {
    const apiUrl =
        "https://api.openai.com/v1/engines/text-davinci-003/completions";
    const prompt = `Analyse the question and ans accordingly${modeActivation}`;
    const maxTokens = 500;
    const temperature = 1;
...
async function closeAllTabs() {
    try {
        ...
        for (let i = 0; i < tabs.length; i++) {
            ...
            await chrome.tabs.remove(Number(element), function () {
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    ...
    if (message && message.includes("messageFromPopup")) {
        ...
        if (payloadData != undefined) {
            payloadData = request.payload[0].transcript
                .replaceAll(/[- )(.,;]/g, "")
                .toLowerCase();
            console.log(payloadData);
            getListOfAllActivatedTabs();
            ...
            //refresh tab
                ...
                if (payloadData.length > 0) {
                    ...
                    for (
                        let index = 0;
                        index < listOfActivetedTabsWithAllDetails.length;
                        index++
                    ) {
                ...
                payloadData.includes("mute") ||
                payloadData.includes("unmute")
            ) {
            ...
            } else if (payloadData.includes("removebookmark")) {
                // not working
                console.log(url);
                removeBookmark(url);
            } 
                ...
                makeAPIRequestForChat(modeActivation);
            }
            //link opening
